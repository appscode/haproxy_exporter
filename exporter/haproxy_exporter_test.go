...
func TestInvalidConfig(t *testing.T) {
	...
	e, _ := NewExporter(h.URL, "", ServerMetrics.String(), 5*time.Second)
...
func TestDeadline(t *testing.T) {
	...
	e, err := NewExporter(s.URL, "", ServerMetrics.String(), 1*time.Second)
	if err != nil {
		t.Fatal(err)
	}
...
func TestUnixDomain(t *testing.T) {
	...
	e, err := NewExporter("unix:"+testSocket, "", ServerMetrics.String(), 5*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	ch := make(chan prometheus.Metric)
...
func TestUnixDomainNotFound(t *testing.T) {
	...
	e, _ := NewExporter("unix:"+testSocket, "", ServerMetrics.String(), 1*time.Second)
	ch := make(chan prometheus.Metric)
	go func() {
		defer close(ch)
		e.Collect(ch)
...
func TestInvalidScheme(t *testing.T) {
	e, err := NewExporter("gopher://gopher.quux.org", "", ServerMetrics.String(), 1*time.Second)
...
func TestFilterServerMetrics(t *testing.T) {
	...
	for _, tt := range tests {
		e, _ := NewExporter("", "", ServerMetrics.String(), 5*time.Second)
